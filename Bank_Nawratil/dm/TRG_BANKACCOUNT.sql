...
BEGIN
    IF (:NEW.AMOUNT < -:NEW.CREDITLIMIT) THEN
        RAISE_APPLICATION_ERROR(-20999, 'The balance ' || :NEW.AMOUNT || ' is over the creditlimit ' || :NEW.AMOUNT);
    END IF;
    ...
    IF (UPDATING) THEN
        ...
        IF(abs(:NEW.AMOUNT - :OLD.AMOUNT) > 10000) THEN
            RAISE_APPLICATION_ERROR(-20998, 'Transaction over 10000 are not allowed.');
        END IF;
    END IF;
END TRG_BANKACCOUNT;